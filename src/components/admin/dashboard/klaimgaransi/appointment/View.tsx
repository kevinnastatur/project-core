...
import { getAppointmentIndex } from "@/services/admin/Appointment";
...
export default function AppointmentView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await getAppointmentIndex(page, perPage, search, status);
        
        if (res.success && res.data) {
          const items = (res.data.data ?? []) as AppointmentData[];
          const filtered = items.filter((item) => {
            ...
            return (
              normalizedStatus === 5 ||
              normalizedStatus === "5" ||
              normalizedStatus === "apptreq" ||
              label.includes("konfirmasi stok")
            );
          });
                  ...
                  {loading ? (
                    ...
                    data.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">{item?.odometer_cust_now ?? 0} km</td>
                        ...
                        <td className="py-3 text-primary font-semibold cursor-pointer whitespace-nowrap pr-5">
                          <Link
                            href={`/admin/dashboard/klaim-garansi/appointment/${item.id}`}
