...
import { getRequestBanIndex } from "@/services/admin/WarrantyClaim";
import Pagination from "@/components/ui/Pagination";
import { showSuccessToast } from "@/helper/toastHelper";

interface RequestBanData {
  id: number;
  po_warranty_code: string;
  applicant_name: string;
  submission_type: string;
  distributor_factory_name: string;
  status: string;
  status_label: string;
}

export default function RequestBanView() {
  const [data, setData] = useState<RequestBanData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await getRequestBanIndex(page, perPage, search, status);
  ...
  const handleExport = () => {
    try {
      ...
      const exportUrl = `${basePath}/api/request-ban/principal/export${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const getStatusBadge = (status: string, label: string) => {
    const statusColors: { [key: string]: string } = {
      "Proses Verifikasi": "bg-yellow-500",
      "Ditolak Ke Toko": "bg-red-500",
      "Klaim Selesai": "bg-green-500",
      "Dibatalkan": "bg-red-500",
    };

    const bgColor = statusColors[label] || "bg-yellow-500";

    return (
      <span className={`${bgColor} px-3 py-2 rounded-full font-bold text-secondary text-xs whitespace-nowrap`}>
        {label}
      </span>
    );
  };
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
            <div className="text-white font-bold text-lg xl:text-xl">
              <p>Request Ban Garansi</p>
              <p className="text-base xl:text-lg text-input-placeholder">
                Menampilkan keseluruhan Data Request Ban Garansi
              </p>
            </div>
          </div>
              ...
              <table className="table-auto text-left text-white w-full">
                <thead className="bg-[#333333] text-sm">
                  <tr>
                    ...
                    <th className="px-5 py-4 whitespace-nowrap">Nama Pemohon</th>
                    <th className="px-5 py-4 whitespace-nowrap">
                      Pengajuan
                    </th>
                    <th className="px-5 py-4 whitespace-nowrap">
                      Nama Distributor/Factory
                    </th>
                    ...
                    data.map((item) => (
                      ...
                      >
                        ...
                        <td className="py-3 text-primary font-semibold cursor-pointer whitespace-nowrap pr-5">
                          <Link
                            href={`/admin/dashboard/klaim-garansi/request-ban/${item.id}`}
