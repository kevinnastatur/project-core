...
import { CreatePromoPrincipal } from "@/services/admin/General";
...
export default function CreatePromotion() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("url", promoLink);
      formData.append("image", image);
      formData.append("status", String(selectedStatus));

      const response = await CreatePromoPrincipal(formData);

      if (response.success === true) {
        showSuccessToast(
          "Promo Ditambahkan",
          `Anda berhasil melakukan upload promo.`
        );
        setHeading("");
        setPeriode("");
        setPromoLink("");
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-5 w-full p-5 text-white min-h-125 mb-20 ">
        ...
        />
        <p className="font-bold">Tambah Promo Baru</p>
        <div className="grid grid-cols-1 lg:grid-cols-2  w-full gap-5  items-start box-container">
          <GeneralInput
            name="banner_name"
            label="Heading Promo"
            value={heading}
