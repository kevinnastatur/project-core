...
import { getPromoIndex } from "@/services/admin/General";
import { BannerItemDetail } from "@/types/general";
import Image from "next/image";
import ButtonAdd from "@/components/ui/button/ButtonAdd";
import { useRouter } from "next/navigation";

type SortKey = "heading" | "image" | "periode" | null;

type SortOrder = "asc" | "desc" | null;

type PromoStatus = 0 | 1;
...
export default function ViewPromo() {
  ...
  const handleAdd = () => {
    router.push("/admin/dashboard/promo/create");
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
            <div className="text-white font-bold text-lg xl:text-xl">
              ...
              <p className="text-base xl:text-lg text-input-placeholder">
                Menampilkan keseluruhan Promosi
              </p>
            </div>
            <div>
              <ButtonAdd label="Tambah Promo Baru" onClick={handleAdd} />
            </div>
          </div>
          ...
          <div className="box-container flex flex-col gap-5">
            <div className=" flex flex-col md:flex-row gap-5  justify-between">
              <div className="relative w-full md:w-70">
                <input
                  placeholder="Cari Berdasarkan Heading Promo"
                  value={searchInput}
            ...
            `}
                      >
                        ...
                        <td className="py-3 text-primary font-semibold cursor-pointer whitespace-nowrap pr-5">
                          <Link
                            href={`/admin/dashboard/promo/edit/${item.id}`}
